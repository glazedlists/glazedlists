...
public class Issue implements TextFilterable, Comparable {
    ...
    public static List loadIssues() {
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Most of the hard work for this bug has been completed. But there are many tasks still required.\n\nCritical for this task's completion:\nBug 123, \"CTP Bind failure should throw IOException\"\nBug 124, \"Resource Block Peer needs concurrency strategy\"\nBug 127, \"Automatic reconnect in Resource Block Peer\"\nBug 128, \"Glazed Lists on network packaging\"\nBug 129, \"ByteCoder for Serializable\"\nBug 130, \"ByteCoder for beans\"\nBug 131, \"Unpublish in Resource Block Peer\"\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Well I guess we can count this done. There are some issues that need to be worked out, but the \nrequired functionality is all ready.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("With the increasing interest in adding JDBC support to Glazed Lists and \noffering other features like paging, I decided about a month ago to dust off \nCachingList.  CachingList would be an invaluable asset in supporting JDBC in \nGlazedLists, but in its current state it is somewhat limited.  I built up a \nunit test which helped to resolve a few perfromance and edge condition bugs.  \nThen I started putting thought towards the more powerful approach to caching \nthat we've had on the backburner since April.\n\nI've been working on a new internal data structure which will allow the memory \nfootprint to be bound by the size of the cache, rather than the size of the \ndata source as it is now.  Since this problem might seem odd to some, I'll \nexplain.  The source event list, whether JDBC-backed or otherwise, will be in a \nstate of constant change.  As such, changes to the source list are modelled \ninternally by CachingList, so that the cache can be maintained more \nefficiently.  This avoids having to clear the cache, or handle offsets to the \nindicies of cached values for each change to the source.  However, the current \nimplementation lacks scalability because the ADT used to model the source list \nhas a memory footprint that is bounded by its size.\n\nSo I'm at the testing stages for that shiny new ADT, and once it is finished, \nI'll be able to move on to the next challenge to make CachingList bigger, \nbetter, faster, stronger: bulk-loading.\n\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The first step of this fix is done!  CachingList memory usage is now bound to \nthe size of the cache.  Running the current performance test with 1000000 \nelements in the source list and a cache size of 1000, the total memory usage  \nfor the test was around 37 Mb.  Running that same test on the old \nimplementation, even if you increase the max heap size to 96 Mb, results in an \nout of memory error.\n\nThe new internal data structure that models the source list also offers \nsignificant performance improvement over the old implementation.  This is not \ncurrently represented in the performance test, but I intend to correct that.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Added a simple internal class called CacheLock that decorates the source lock \nin such a way that calls to readLock() return writeLock().");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("So far as I know, this is done?");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've done another round of work on SWT for Glazed Lists and I have some working code.\n\n- EventTableViewer is 90% complete\n- EventTableViewer supports checkable objects\n- TextFilterList is 80% complete\n\nTo Do:\n - finish TextFilterList, refactor with respect to shared code in swing/TextFilterList\n - finish EventTableModel, allow the TableFormat to be changed\n - further research sorting integration in EventTableViewer");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The TextFilterList now works very well with James' new DefaultTextFilterList class.\nI am almost ready to announce formal SWT support in Glazed Lists.\nThis will mean that the Glazed Lists release jars will all include SWT classes.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("For buffer sizing, I recommend allowing up to X gigabytes allocated via random access files.\n\nFor multiple threads, I recommend that since CTP already starts its own thread when created, all access \nshall be marshalled through that thread, similar to SwingUtilities.invokeLater() and \nSwingUtilities.invokeAndWait(). For outgoing updates, this same thread will be used. Therefore it is \nnecessary that no 'long' operations are performed during list notifications. I believe this is already the \nstate of ListEventListener.\n\nBut come Glazed Lists on the XXX, XXX may always need its own thread.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've fixed a reading bug in the CTPConnection class and now our single test case passes!\n\nLeft to do: \n - close() asynchronously. We will probably need to add another state \"PENDING_CLOSE\" to manage \nsending the last bits and pieces of message on a close(). Make sure that close() doesn't call flush() but \ninstead requestFlush(), since this method may be called on any thread\n - buffer resizing\n - test the heck out of it");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Async close and Async send are now implemented. To do:\n - buffer resizing\n - tests for the following criteria\n     - simultaneous connections\n     - large data\n     - frequent data\n\nPlus some of the following things need consideration:\n - timeouts! timeouts! timeouts!\n - configurability");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText(" - buffer resizing: DONE!\n\nThe main stuff in CTP is done. All that's left is little things and test cases.\nOne new problem: The server doesn't complain if it cannot bind.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Some exceptions to investigate:\n\n   [junit] WARNING: Connection Manager failed to start, Address already\nin use:\nbind\n   [junit] Dec 3, 2004 4:08:02 PM ca.odell.glazedlists.net.CTPStop run\n   [junit] INFO: Sever stopping with 0 active connections\n   [junit] Dec 3, 2004 4:08:03 PM\nca.odell.glazedlists.net.CTPConnectionManager\nstart\n   [junit] INFO: Connection Manager ready, listening on\n0.0.0.0/0.0.0.0:5001\n   [junit] Dec 3, 2004 4:08:04 PM ca.odell.glazedlists.net.CTPConnection\nsendRe\nponse\n   [junit] INFO: Accepted connection from 127.0.0.1\n   [junit] Dec 3, 2004 4:08:04 PM ca.odell.glazedlists.net.CTPConnection\nhandle\nesponse\n   [junit] INFO: Established connection to localhost\n   [junit] Dec 3, 2004 4:08:04 PM\nca.odell.glazedlists.net.CTPConnectionToClose\nrun\n   [junit] INFO: Closed connection to localhost\n   [junit] Dec 3, 2004 4:08:04 PM\nca.odell.glazedlists.net.CTPConnectionToClose\nrun\n   [junit] INFO: Closed connection to 127.0.0.1\n   [junit] Dec 3, 2004 4:08:04 PM ca.odell.glazedlists.net.CTPShutdown\nrun\n   [junit] INFO: Cleaning up listening socket and closing 2 connections\n   [junit] ------------- ---------------- ---------------\n   [junit] Testcase:\nTestSendLargeString(ca.odell.glazedlists.net.CTPChunkTest)\n       Caused an ERROR\n   [junit] null\n   [junit] java.lang.IllegalStateException\n   [junit]     at\nca.odell.glazedlists.net.CTPConnectionManager.invokeAndWait(CTPConnectio\nnManager.java:172)\n   [junit]     at\nca.odell.glazedlists.net.CTPConnectionManager.stop(CTPConnect\nonManager.java:216)\n   [junit]     at\nca.odell.glazedlists.net.CTPChunkTest.tearDown(CTPChunkTest.java:48)\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("ChunkTransferProtocol has been implemented, but theres some roughness around the edges:\nBug 123, \"CTP Bind failure should throw IOException\"\nBug 125, \"Many calls to Bufferlo.write()\"\n\nRegardless, the core of this objective has been met. New issues have been created to track the \npolishing details.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("According to my analysis, Glazed Lists 1.0 is 'ready'. But I think we should do a few more things before \nwe release ONE-DOT-OH:\n\nBug 146: Cannot deregister Event*Model as Listener\nBug 112: EventTableModel.setTableFormat() clears selection\nBug 147: Swing JTable selection can result in a RuntimeException\nBug 140: Potential DoS in Network Glazed Lists\nBug 141: NetworkList could suck up all ram\nBug 144: Glazed Lists should recover from RuntimeExceptions\n\nWhen this is all done, we're at ONE.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("See dev@ thread, \"Glazed Lists 1.0 Status\" ...\n\n----- ----- ----- ----- ----- ----- ----- \nRob's must fix for 1.0 list:\n   CollectionList\n\n----- ----- ----- ----- ----- ----- ----- \nKevin's must fix for 1.0 list:\n\nIssue 47 - ThreshholdList\n\nIssue 134, Issue 137, Issue 152 - The SWT Bugs...\n\nSwing - selection inversion (to be on par with SWT)\n\nIssue - Tutorial 1.0:\n    The DocBook tutorial has image references which are placeholders because\nI was offline when I wrote it in the first place, these should be\ncorrected.\n    Update the tutorial images to something other than KDE since that\nmachine has been rebuilt.\n     We might want to look into building a custom XSLT for our HTML tutorial.\nUsing the default html XSLT provided with the DocBook download, the\nentire tutorial is on one page.  There also feels like there might be\ntoo many tables of contents as there is one for each chapter and a\nprimary one.  If each chapter had its own page that would be fine.\n     There's also the Ant task that Jesse had mentioned in Issue 135 that we\nshould look into for Ant-based document compilation.\n\nIssue - Release Checklist:\n    ...here's Jesse's Glazed Lists Release Checklist. It's not in CVS but I have it in a private version control, \nwith the project website.\n_ JAR file has Implementation-Version tag\n_ CVS Tag the release\n_ Java.net Announcement\n_ Java.net News\n_ Documents & Files archive\n_ Issue Tracker new version\n_ Release Notes, readme.html in source distribution\n_ Webpage\n_ API Docs online\n_ Tutorial ?\n\nIssue - 3 outstanding errors in the test cases:\n* CTPChunkTest fails due to premature closure on testManyStrings(). If it doesn't fail for you I will send \nyou the stack trace, but it's really quite long so I won't include it here.\n* PeerConnectionTest's testPeerConnection fails due to a bind error, address already in use.\n* CTPConnectionTest fails due to connection refused with no further info.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've revised the list of bugs to fix for 1.0, in response to emails from Rob and Kevin...\n\nMOST IMPORTANT:\nBug 140: Potential DoS in Network Glazed Lists\nBug 141: NetworkList could suck up all ram\nBug 144: Glazed Lists should recover from RuntimeExceptions\nBug 155: Tutorial needs updating\nBug 156: NetworkList tests fail\n\nEVERYTHING ELSE FOR 1.0:\nBug 47: ThreshholdList\nBug 106: Collection extracting List\nBug 134: SWT Selection Event List for SWT\nBug 137: SWT Viewer class for Sliders for ThresholdList\nBug 146: Cannot deregister Event*Model as Listener\nBug 152: SWT package lacks Combo support\nBug 154: Swing - selection inversion (to be on par with SWT)");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Another bug that we can't let slip through the cracks is Issue 96 - Multiple \nSources on a CompositeList which affects our ListEventPublisher dependency \nsystem.\n\nAs an additional point on the Release Checklist:\nUpdate the demo jar...not to be confused with Issue 150, this refers simply to \nupdating the jar that our demo app relies on to make sure it is using (and \nshowing off) the latest and greatest that GlazedLists has to offer.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        ...
        currentIssue.setComponent("glazedlists");
        currentIssue.setVersion("0.9.1");
        currentIssue.setAssignedTo("jplemieux");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("James has been hard at work for a long time, now it's officially his.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("For this to work we need a single message called \"Fetch All and Subscribe\".\n\nThis message is sent by a client to a resource. The resource is read-locked and copied to the \nsubscriber. Then the client is subscribed to the resource, then the read-lock is released. This allows the \nclient to get a resource without potentially missing any update events.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We will definitely need read locking implemented for a lot of stuff that serializes the whole list down.\n\nSince the FileList takes a snapshot of the list that is readable, the NetworkList could use this snapshot \nrather than reserializing everything on every 'fetch-all' request.\n\nThis would require some functionality in FileList to acquire such a snapshot as a series of ByteBuffers. \nThe ByteBuffers would be locked from changes by the FileList during this process.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I'm going to proceed with implementation without cryptography. I've tried to do some analysis and \ncrypto is a very hard problem, especially in the context of a malicious middle-man.\n\nSome problems:\n - checksumming all elements individually allows a malicious user to reorder the list\n - checksumming the entire list requires a new checksum for each change\n - checksumming the deltas is insufficeint for new subscribers\n\nAlso note that every checksum should be done on something padded with a big fat salt or something \nsince I have a feeling that short data is easier to decrypt without a private key.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've done some initial work.\n\nAll blocks include:\nBlock Size\nResource Name\n\nBlock types and their additional information:\n1. Subscribe\n(Optional) last seen update ID\n\n2. Subscribe Confirm\n(Optional) Snapshot\nUpdate ID\n\n3. Unsubscribe\n\n4. Update\nUpdate ID (w update unique ID)\nEvent Delta\n\nHow many recent updates on disk?\nA certain temporal value, say last 5 minutes");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The primary data types:\n\nPEER\n\nRESOURCE\n - List of Connections for each listener\n - connection of source\n\nCONNECTION\n - map of resources being published\n - map of resources subscribed to\n - CTPConnection object\n Incoming Subscribe: create appropriate in publish, delegate\n Incoming SubscribeConfirm: activate in resource\n Incoming Update: activate in resource\n Incoming Unsubscribe: remove from publish, remove from resource\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've got ResourceBlockPeer working. It's not complete, but it's working.\n\nLeft to do:\n - unpublish\n - status listeners for resources (offline, online, update count, other stats)\n - auto reconnect\n - peer to peer\n\nOne particularly annoying part of this problem is that each of my three data objects is keeping pointers \nto each of the other types of data objects.\n\nSo - connection keeps a list of resources and the parent peer\n  resources keeps a list of connections and the parent peer\n  peer keeps a list of resources and a list of connections\n\nThis is difficult to code with respect to getting it all in sync all the time.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This task is essentially finished. We replace it with some other tasks:\nBug 124, \"Resource Block Peer needs concurrency strategy\"\nBug 127, \"Automatic reconnect in Resource Block Peer\"\nBug 131, \"Unpublish in Resource Block Peer\"\nBug 132, \"Resource Block Peer needs P2P\"\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("pkuzel");
        description.setWhen(null);
        description.setText("Conceptually I agree.\n\nI tried to use ComposedList and it fails because it does not support set()\noperations. That is crucial once wrapped into EventTableModel.\n\nThe behaviour can be improved on CompositeList side or EventTableModel. The\nfirst approach could be tricky it's better to document the CompositeList as\nread-only list. For the second:\n\n    public void setValueAt(Object editedValue, int row, int column) {\n        // ensure this is a writable table\n        if(tableFormat instanceof WritableTableFormat) {\n            source.getReadWriteLock().writeLock().lock();\n            try {\n                WritableTableFormat writableTableFormat =\n(WritableTableFormat)tableFormat;\n                // get the object being edited from the source list\n                Object baseObject = source.get(row);\n\n// >>>>>>>>>>\n                // tell the table format to set the value based on what it knows\n                Object updatedObject =\nwritableTableFormat.setColumnValue(baseObject, editedValue, column);\n                // update the list with the revised value\n                if(updatedObject != null) {\n                    source.set(row, updatedObject);\n                }\n// >>>>>>>>>>\n            } finally {\n                source.getReadWriteLock().writeLock().unlock();\n            }\n        // this is not a writable table\n        } else {\n            throw new UnsupportedOperationException(\"Unexpected set() on\nread-only table\");\n        }\n    }\n\nshould not be marked block improved:\n\n// >>>>>>>>>>\n                // tell the table format to set the value based on what it knows\n                Object updatedObject =\nwritableTableFormat.setColumnValue(baseObject, editedValue, column);\n                // update the list with the revised value IF GOT DIFFERENT VALUE\n                if(updatedObject != null && !updatedObject.equals(baseObject)) {\n                    source.set(row, updatedObject);\n                }\n// >>>>>>>>>>\n\n\nHigher level question should EventList fire on members equals() change? Imagine:\n\n  Item i1 = new Item(\"1\");\n  Item i2 = new Item(\"2\");\n  EventList el = ....\n  el.add(i1); // fires\n  el.add(i2); // fires\n\n  i1.setEqualityValue(\"3\"); // equality changed, list contains same instances\n                            // but different values!\n\n\nThe list cannot catch this situation so it cannot be handed at list level.\nClients such as EventTableModel can simulate it but all clients must be\nconsistent. It's rather application design decision but I'm not sure all are\naware of the issue. It should be documented at the EventTableModel client and it \nshould probably support two modes: current one and silent mode when it never\nsource.set()s.\n\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Petr brings up a very important point: CompositeList is not compatible with writable tables.\n\nUnfortunately, we must call the set() method, even if the updated value has the same reference. This \nset() causes everything to be repainted which is very necessary. One simple example: if the updated \nvalue is in a SortedList, the change may cause the value to be moved to a new location. This is only \npossible with a set() invocation.\n\nYet I still see how it is critical to make the CompositeList work in this situation. I think a satisfactory \nsolution is to make CompositeList support the following methods: \n - set(index, value)\n - remove(index)\nBecause in these cases, index is not ambiguous. For anything without an index, CompositeList shall \ncontinue to throw a RuntimeException. Similarly for add(index), because that index is also ambiguous.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("pkuzel");
        description.setWhen(null);
        description.setText("> One simple example: if the updated  value is in a SortedList, the change may\n> cause the value to be moved to a new location. This is only  possible with a\n> set() invocation.\n\nAs a user I would consider it bug. I'd expect resorting only on leaving editing\nof the table row. But I have not checked other database UIs, such as Access.\n\nAnyway I have fixed the problem by subclassing the EventTableModel and\nredefining the setValue(). Well, it does not resort on leaving row. It's\nprobably also 'feature', user can explicitly ask for resort by refresh or better\nusing standard sort gesture. In my UI view sort-liveness is not a plus.\n\nI'd add explicit SortedList.sort() method. Then clients can hint on resorting\ninstead of being dependent on receiving listChanged() events. Of course client\ncan call it from listener if it knows that it's a good thing (tm). A method\nSortedList.setAutoResort(boolean) could also help.\n\n>  - set(index, value)\n>  - remove(index)\n\nIt cannot hurt and makes sense.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've added set() and remove() method support. This should cause the CompositeList to start working \nwith an editable EventTableModel.\n\nNon-live sorting is not possible for reasons I shall explain here.\n - The SortedList stores values in sorted order: [A E I O U]\n - Suppose an element is changed which destroys the sorted property, set(2, B): [A E B O U]\n - Now wen elements are inserted, sorting is broken. add(C): [A E B C O U]\nSince comparisons are done between elements, changes to an element without relocating it can cause \nthe entire sorted property of the table to be destroyed. This can only be fixed with setComparator().\nTherefore it is necessary for the SortedList to stay sorted. When an element is changed, it must be \nmoved to an appropriate new location if necessary.\n\nNon-live sorting can by implementing \"DeltaList\" which is described in issue 24.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("James Lemieux has implemented the fix to this bug! Kevin is currently working on a new class to \nreplace CompressableList that will manage state for these newer faster methods.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Glazed Lists will not support JDBC in the near future. I am marking this priority down to 5.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        ...
        description = null;
        description = new Description();
        description.setWho("jplemieux");
        description.setWhen(null);
        description.setText("This cache is an interesting idea. The motivation centers around the observation\nthat users may frequently use the backspace key when typing search terms. When\nthey do, there is a possibility of a cache hit.\n\nAn interesting version of this optimization can be easily implemented now that\nKevin has delivered his Barcode ADT into the stream. Specifically, if the\nresults of a particular filter are cached, then we have two precisely equivalent\nforms we can use to describe the list entries that match a filter.\n\nFor each filter we can either cache the indices of matching elements OR the\nindices of non-matching elements. Whichever list is more compact will determine\nwhich form we elect to cache. It can vary for each filter, so filters that are\n\"well matched\" will want to record the small list of non-matched indices.\nAlternatively, filters that are \"poorly matched\" will want to record the small\nlist of matched indices from the source list.\n\nFun problems occur around the following variables:\n\na) how should a cache size be chosen/limited? Number of cache entries? Amount of\nRAM?\n\nb) is there a way to estimate whether the cost of RAM for a given cache entry is\nworth the speed improvement a cache hit would give us? I realize that involves\nthe user specifying their preference of speed vs. memory.\n\nc) for a real world implementation, it could be an idea to make cache updating a\nbackground tasks maintained by a low priority thread. The idea would be that a\nmaintenance thread could attempt to calculate and store cache entries if the\nuser pauses long enough between keystrokes, but would not prevent further\nfiltering from taking place. True, the cache may not always contain every filter\nthat was encountered, however, we never block the user from proceeding just for\nthe benefit of creating a cache entry for a given filter... an activity that is\nnot guaranteed to give them any benefit.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I think you've misunderstood my suggestion, James, but you've given some good discussion.\n\nI was thinking more about doing caching while the handleFilterChanged() and listChanged() events are \ntaking place. In these methods, we repeatedly run filterMatches() to test if the set of filter strings is \ncovered by a current object's strings.\n\nFor many sets of data there is much repetition in the exact values in the filter Strings. For example, the \nIssues database repeats some strings very frequently: \"FIXED\", \"CLOSED\", etc. If the user types \"fix\", \nthen we could get a hash of each matching value (ie. hash(\"FIXED\")) and then consult the list of \nmatching hashes before doing the regex search.\n\nThis is just an idea, I think there are a lot of other optimizations that could be more fruitful.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("A very talented programmer named James Lemieux has lent some lines of code to our cause. Now \nTextFilterList uses Boyer-Moore, the 'fastest known' text searching algorithm.\n\nHere's the performance gain:\n  Filtering the Mozilla bug database went from 14.2s to 8.4s.\n  Filtering my MP3 collection went from 0.77s to 0.36s.\n\n\nslackette:glazedlists$ java -Xmx300000000 -cp classes/ \nca.odell.glazedlists.swing.test.TextFilterPerformance mozillabugs.tfpt \nReading input\nRead input, 260836 elements\n\nFilter 0: frameset, expect: 327\nFilter 1: java PC @sun.com, expect: 320\n\nRunning... done. Time: 8417ms\n\n\nslackette:glazedlists$ java -Xmx300000000 -cp classes/ \nca.odell.glazedlists.swing.test.TextFilterPerformance mp3collection.tfpt \nReading input\nRead input, 5138 elements\n\nFilter 0: Limp, expect: 79\nFilter 1: Bizkit, expect: 79\nFilter 2: file 128 44100, expect: 508\nFilter 3: iTunes Music jessewilson, expect: 2568\n\nRunning... done. Time: 354ms\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("CompressableList uses multiple nodes when one will do");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("This task is perfectly inline with the work I am doing for CachingList.  I am \ncreating a deritive of SparseList that will significantly improve performance \nand scaling.  Once I've completed that first step towards improving \nCachingList, I will move on to this.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The new SparseList ADT for CachingList is complete so I am turning my attention \nhere.  Here are some of the ideas I am going to build on for this enhancement.\n\n- None of the GlazedLists make use of the generality of CompressableList \n(formerly known as SparseList).  There are only ever 3 values stored in a \nCompressableList: true, false, and null.\n\n- the new challenging case to handle will be inserting 'false' in the middle of \na 'true' node that has compressed more than one 'true'.  This results in not \none but two new nodes being created, as it splits the 'true' node.  Naively, it \nmight make sense for balancing reasons to add one new node to each subtree.  \nHowever, this can result in significantly more AVL rotations than are necessary \nbecause the tree would have to be validated for the AVL property twice.  Adding \nboth nodes to the right or left subtree may do more rotations than one AVL \nvalidation with the naive approach, but there is no risk of an AVL rotation \nbeing 'reversed' by having two seperate validation passes.\n\n- A possible base case for node splitting occurs when the node to split has at \nmost one child in each subtree.  In this case the new value becomes the root, \nthe split old values the root's children, and the old value's children become a \nleft leaf on the left subtree and a right leaf on the right subtree \nrespectively.  This case always results in an AVL subtree and possibly no AVL \nrotations at all.\n\n- Another interesting problem is node collapsing on deletes or sets.  Removing \nor changing the value of a node may result in several nodes of the same value \nbeing adjacent to each other.  If those nodes do not have nulls between them, \nthey need to be collapsed into one node.\n\nThis is definitely a fun problem and I'm really looking forward to working on \nit!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Well after discussing this with Jesse, we arrived at the conclusion that the \nrare use of Boolean.FALSE (which is only used in UniqueList during event \nprocessing) would be more easily refactored than implementing the \nCompressableList revisions with value awareness.\n\nWith that in mind, I have changed my goals for this task to implement \nCompressableList in the following manner:\n\nThe list contains only values and nulls, but does not distinguish between \nvalues.  A value is simply represented by Boolean.TRUE.  Internally, value \nblocks are joined whenever possible.\n\nCurrently, I have a working implementation that satisfies most of those goals.  \nNot bad for 2 days work, but there's still more testing to be done and a few \nmore things to add.\n\n- Delete and Set are lazy about joining value blocks in the list.  Currently \ncompression only occurs on Insert.  This needs to be implemented.\n\n- There should be a new method on the public interface that allows bulk adding \nof values in the spirit of addNulls(index, length).  This will save any calling \nclasses from the need to use a for loop to add values individually.  Minimal \nsavings, I admit, but the convienience is nice.\n\n- Write more test cases.\n\n- Optimize, optimize, optimize!\n\nWell, no time like the present to start...\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The changes are in CVS under the name 'BooleanList' for now.  UnqiueList still \nneeds to be adjusted to removed dependencies on Boolean.FALSE before it can \nmake use of the new code.  This bug will stay open until all of \nEventSelectionModel, AbstractFilterList and UniqueList can all use the new \nstructure and the old CompressableList has been replaced by the code in \nBooleanList.\n\nThings are set to get even better once the next ADT, codenamed 'Barcode' is in \nplace.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("This bug is being marked as closed although there is still a dependency on the \nmulti-node CompressableList to be found in UniqueList.  As such, issue 114 was \ncreated to track the refactoring of UniqueList to remove dependency on \nCompressableList.  No other GlazedLists are dependent on CompressableList \nanymore.  The replacement ADT, Barcode, satisfies the intent of this bug.  \nBooleanList, the interm CompressableList replacement, can now be removed from \nthe source tree.\n\nThis fix permits performance enhancements for text filtering that we were \npreviously unable to complete.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("migrationkit: GONE!\nJavadoc warnings: DONE!\nWebStart demo: READY!\n\nNow the only thing I think would be worthwhile is to change the lame threading model. This will require \na little code and a lot of testing. Simply remove all the auto-locking methods, and make this a user \nresponsibility. This is now in bug 97.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Released on October 24, 2004 at 12:08 am!\n\nThanks to everyone for having patience with me on coding day.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've implemented most of the required functionality. Still required is CompositeList support, and \nperhaps some optimization.\n\nSee bug 96 for for CompositeList support.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The issues.xml stuff has been refactored and updated.\n\nI have modified the Issue class and created a class IssuezillaXMLParser, this class now outputs Java code \nto reconstruct the Issues set. The generated Java code is then put into an Issues.java file.\n\nThe size is now larger (almost 50K) but this includes nearly 100 issues from the Glazed Lists issues \ndatabase.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(96));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Multiple sources for CompositeList");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("There is no support in EventListPublisher in the CompositeList. This will be necessary!\n\nA good first step would be to create a test case that fails. This is a good way to light a fire under a \ndeveloper's ass!");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(97));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P2);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Multithreading is not intuitive");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Glazed Lists threading model currently acquires write locks automatically.\n\nThis is dangerous (can lead to deadlocks if used improperly)\nThis is non-intuitive (locks required on write but not read)\nThis is slow (several locks acquired when EventLists are chained)\n\nWe should remove the built-in lock acquisitions and replace it with documentation.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Multithreading is officially fixed. The new scheme works as follows.\n\nUser code that requires only one thread will no longer do any locking.\n\nUser code that requires multiple thread support must ALWAYS do manual locking:\nmyList.getReadWriteLock().readLock().lock();\ntry {\n     // access myList\n} finally {\n     myList.getReadWriteLock().readLock().unlock();\n}\n\nThe above code is for READING, for writing, replace readLock() with writeLock().\n\nThis is simpler and better. I am much happier with it.\nDoing automatic threading is a neat idea, but too easy to do wrong, especially with many series of \ntransformations.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(98));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Performance 'team' idea");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Glazed Lists needs a high-performance 'team' which is responsible for:\n - profiling\n - computer science optimizations (\"smart\" optimizations)\n - java optimizations (\"micro\" optimizations)\n - tracking performance history\n - performing comparison tests vs. other libraries, ie. JDNC, TableSorter\n - performing comparison tests vs. theoretical optimal\n\nBy making performance more formal, I think we will be able to continuously improve the performance \nof Glazed Lists. Because Glazed Lists is an API and not an end product, I believe that performance will \nbe a critical factor in its success.\n\nThis task is complete when:\n - we have a performance status webpage\n\nIt would be nice to have spaces on this webpage for:\n - we have performance statistics on BasicEventList vs. ArrayList vs. Common Collections' TreeList\n - we have performance statistics on SortedList vs. Arrays.sort() vs. Quicksort\n - we have a general performance test program that we time, should take ~30 minutes or so to run and \nexercise all of the APIs with timed sections for each");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(99));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("issues@glazedlists");
        currentIssue.setShortDescription("Demo app description table repaints twice for each selection change");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Currently the description table at the bottom of the demo app repaints itself \ntwice for each selection change.  The description table header shakes rather \nviolently during the repainting as well.  Jesse and I have already discussed \nthis issue, but I figured that I'd toss it in anyway.  It's easier to keep \ntrack of this way.  \n\nAlso, it might be worthwhile to add 'demo' as an issue subcomponent for future \ndemo-related issues.  I put this one in 'www' because that seemed to be the \nbest fit.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Fixed! Now three vertical scrollbars are always shown:\n - for the users table\n - for the issues table\n - for the description table");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(100));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("TableComparatorChooser needs listener");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We need a listener for when the sort order changes so that classes can retain the sort order \npermanently.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("There's a new listener! It works good!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(101));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Test classes should be in test/ tree");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Move the test classes from the source tree to a new test/ tree.\n\nThis simplifies annoying naming problems.\nPlus it enables us to test friendly-scoped classes.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Moved!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(102));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P2);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Sublist follows an element through sort, filter, other changes");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("A Sublist containing one element should continue to contain that element as long as that element is in \nthe source list. This should survive all list mutations, including sort, filter, add, remove etc.\n\nThis is for Eric Lehman's problem.\n\nThis will require a JUnit test and some work on how sublist moves when indices change.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Fixed! Now Sublist is resistent to change!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(103));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("BeanTableFormat, BeanFilterator broken for read-only properties");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Because these classes use the PropertyDescriptors class, they do not work for read-only properties. We \nwill have to roll-our-own PropertyDescriptor-like class that works in either case.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've created a new class called \"BeanProperty\" that solves this problem rather elegantly!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(104));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("FEATURE");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Popularity List");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We should create a PopularityList where elements are sorted by the number of duplicates of elements \nthat are in the list. We can use SortedList and UniqueList to accomoplish this goal.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I needed to do a handful of work on UniqueList to make sure it fires the appropriate non-mandatory \nchange events. Now PopularityList is all done and it works great!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(105));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("INVALID");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Create package-renamer for Spring RCP integration");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I would like to build a package-renamer to allow Glazed Lists to be integrated into the Spring RCP \nproject. This project has a lot of mindshare and would benefit from Glazed Lists code being embedded \nwithin it. Similarly Glazed Lists may benefit from a higher adoption rate.\n\nWe need to create a package renamer Ant task that converts ca.odell.glazedlists.* into org.spring.lists \nor whatever.\n\nWe shall continue to maintain the Glazed Lists project in its current package. Spring RCP will be an \nOEM-like build that will hopefully be embedded in their project. It will simply be another means of \ndistribution of the Glazed Lists code.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This isn't what we want afterall. See Bug 107.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(106));
        currentIssue.setStatus("STARTED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        currentIssue.setVersion("current");
        currentIssue.setAssignedTo("reden");
        currentIssue.setSubcomponent("core");
        currentIssue.setIssueType("FEATURE");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Collection extracting List");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This Task obviously needs a better name. Here's my idea:\nSuppose you have an EventList of Records, and you would like to get an EventList of Songs, where each \nRecord contains 0 or more songs.\n\nGlazed Lists should have an abstract list that makes this easy. It could use the filtering data structures \nin reverse order to do mapping. Methods to expect: \n\n    getChildCount(int memberIndex);\n    getChild(int memberIndex, int childIndex);\n\nThis is particularly awesome when coupled with UniqueList. Suppose you have an EventList of \nCustomers, and each customer has 0 or more recently purchased items. Then you could get a list of all \nrecently purchased items from the list of customers.\n\nThis is particularly practical when the list of customers is dynamic. So you can compose lists that match \n\"Show me all the unique purchases from the list of customers that match this threshold.\"");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This is Rob Eden's new task, whether he wants it or not!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("A very simple use case takes a list of words  { \"Jesse\", \"Kevin\", \"Rob\", \"James\" } and convert it into a list \nof characters { \"J\", \"e\", \"s\", \"s\", \"e\", \"K\", \"e\", \"v\", \"i\", \"n\", \"R\", ... }\n\nBasically what this will require is methods to map the expanded index to the original index and back. \nSo we'd need to answer the questions \"Get me the 9th character\" or \"Give me the character index range \nfor the 2nd word\"\n\nI've got some ideas on how the implementation of this should go down.\n\nWe could use a Barcode where each word element gets a single black element and a white element for \neach component in its character list. Then we could use the heavily-optimized mapping methods of \nBarcode to do the adjustments. These methods are getWhiteIndex(), getBlackIndex() and these could be \nused to do the mapping.\n\nThe only tricky part would be ensuring a proper alternation. This might require a black element for each \nword, even if that is unnecessary.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This is not a DEFECT it's a new FEATURE.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("reden");
        description.setWhen(null);
        description.setText("A basic implementation is now working and the interfaces have been finalized (although they still need \nto be peer reviewed).\n\nStill to go:\n  - Documentation\n  - Performance optimization (currently lookups are relatively expensive)\n  - JUnit test (demo/test application is developed, but not well suited for unit testing)\n  - Review by other devs to make sure I understand all the locking, etc.\n     (esp. since it's my first new list. :-)");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("reden");
        description.setWhen(null);
        description.setText("Checked initial version into CVS.\nStill to do:\n  - JUnit tests\n  - Can it use Barcode??");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("reden");
        description.setWhen(null);
        description.setText("Revised version using Barcode and unit tests checked in. Will be marked FIXED after review.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(107));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("issues@glazedlists");
        currentIssue.setShortDescription("Spring RCP catalyst class");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("As described in this forum, we're ready to put Glazed Lists into use in Spring RCP:\nhttp://forum.springframework.org/viewtopic.php?t=2087\n\nThe SpringRCP CVS already includes Glazed Lists source .zip. We need to write some classes in the \nSpring codebase so that they can take better advantage of Glazed Lists.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've got some advice on this from Peter De Bruycker. Here are his comments verbatim:\n\nWell, I've been working on a ListSelectionDialog, to let the user choose\nan item from a dialog. It's build after the \"Open type\" dialog in\neclipse. First I coded the live filtering and sorting myself, until I\nstumbled upon GlazedLists. Much simpler ;-) The ListSelectionDialog has\nbeen committed to the sandbox area of Spring RCP, so make sure to check\nit out, and feedback any ideas.\n\nI think we could use GlazedLists in Spring RCP in the following areas:\n- creation of JComboBoxes for enums\n- JTable support\n- JComboBox and JList support\n- bound List controls\n- the PetClinic sample application (I think a view with a table, with a\nListTableModel, to show of the sorting/filtering/...)\n\nPerhaps the ComboBoxAutoCompletionSupport classes could be donated to\nGlazedLists, as it's more about JComboBoxes than rich client\ndevelopment. Perhaps it would fit better in GlazedLists than in Spring\nRCP.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(108));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("EventSelectionModel is not a ListSelectionModel");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The EventSelectionModel class should expose the ListSelecitonModel API directly instead of via a \ngetListSelectionModel() member.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Fixed! Now the EventSelectionModel exposes the ListSelectionModel API directly.\n\nIn continues to include the method getListSelectionModel(), which has been deprecated.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(109));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P5);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Caching in IndexedTree");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("IndexedTree exposes only one really important read method, \n    get an Object at the specified index\n\nIf the IndexedTree is read frequently and rarely written, we have an opportunity to optimize.\n\nWhat if we added the following:\n - an integer 'time' that shows the current time\n - an ArrayList of Objects such that the Object at i in the IndexedTree MIGHT be in the ArrayList at i\n - a parallel ArrayList of Integers. If the ArrayList entry at i was most recently set at time t, then this \ninteger array would contain time t.\n\nThen when we do a lookup, we save the time and value in the two arrays. Next time we do a lookup, we \ncan first consult the time. If it is up-to-date, we can just use the IndexedTree value. If it's not, we can \ndo as we normally would and then save the updated value.\n\nWhenever the list changes, we update the time.\n\nAny comments?");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(110));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P1);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("EventSelectionModel throws exception on lead select");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("To reproduce:\n - run demo app.\n - select item 30\n - filter to \"sparse\"\n - shift-click item 90\n\nResult:\napple.awt.EventQueueExceptionHandler Caught Throwable : java.lang.IndexOutOfBoundsException: \nInvalid range for selection: 29-9, list size is 10\njava.lang.IndexOutOfBoundsException: Invalid range for selection: 29-9, list size is 10\n        at \nca.odell.glazedlists.swing.EventSelectionModel$EventListSelectionModel.setSubRangeOfRange(EventSele\nctionModel.java:425)\n        at \nca.odell.glazedlists.swing.EventSelectionModel$EventListSelectionModel.setLeadSelectionIndex(EventSel\nectionModel.java:744)\n        at javax.swing.JTable.changeSelectionModel(JTable.java:1492)\n        at javax.swing.JTable.changeSelection(JTable.java:1548)\n        at \njavax.swing.plaf.basic.BasicTableUI$MouseInputHandler.adjustFocusAndSelection(BasicTableUI.java:\n522)\n        at javax.swing.plaf.basic.BasicTableUI$MouseInputHandler.mousePressed(BasicTableUI.java:494)\n        at apple.laf.AquaTableUI$MouseInputHandler.mousePressed(AquaTableUI.java:93)\n        at java.awt.AWTEventMulticaster.mousePressed(AWTEventMulticaster.java:222)\n        at java.awt.Component.processMouseEvent(Component.java:5163)\n        at java.awt.Component.processEvent(Component.java:4963)\n        at java.awt.Container.processEvent(Container.java:1613)\n        at java.awt.Component.dispatchEventImpl(Component.java:3681)\n        at java.awt.Container.dispatchEventImpl(Container.java:1671)\n        at java.awt.Component.dispatchEvent(Component.java:3543)\n        at java.awt.LightweightDispatcher.retargetMouseEvent(Container.java:3527)\n        at java.awt.LightweightDispatcher.processMouseEvent(Container.java:3239)\n        at java.awt.LightweightDispatcher.dispatchEvent(Container.java:3172)\n        at java.awt.Container.dispatchEventImpl(Container.java:1657)\n        at java.awt.Window.dispatchEventImpl(Window.java:1606)\n        at java.awt.Component.dispatchEvent(Component.java:3543)\n        at java.awt.EventQueue.dispatchEvent(EventQueue.java:456)\n        at java.awt.EventDispatchThread.pumpOneEventForHierarchy(EventDispatchThread.java:234)\n        at java.awt.EventDispatchThread.pumpEventsForHierarchy(EventDispatchThread.java:184)\n        at java.awt.EventDispatchThread.pumpEvents(EventDispatchThread.java:178)\n        at java.awt.EventDispatchThread.pumpEvents(EventDispatchThread.java:170)\n        at java.awt.EventDispatchThread.run(EventDispatchThread.java:100)");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("These indices are now adjusted properly when a list change occurs.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(111));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("TableFormat should have a getColumnClass() method.");
        description = new Description();
        description.setWho("pdbruycker");
        description.setWhen(null);
        description.setText("TableFormat should have a getColumnClass() method.\n\nIf you're registering cell renderers on a JTable using the setDefaultRenderer \nmethod, they are not being used. The problem is that EventTableModel doesn't \noverride the default getColumnClass() method (which returns Object.class) of \nAbstractTableModel. This method should be implemented by returning \ntableFormat.getColumnClass(column).\n\nCurrent workaround: create a subclass of EventTableModel that overrides the \ngetColumnClass method, and return the correct type.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've updated the description to be more verbose.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("*** Issue 113 has been marked as a duplicate of this issue. ***");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Rob's first task is already in CVS.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("reden");
        description.setWhen(null);
        description.setText("Fixed by adding an \"AdvancedTableFormat\" interface which adds a getColumnClass(int) method.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(112));
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("EventTableModel.setTableFormat() clears selection");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This does not need to clear the selection. Since the selection is row-based and the TableFormat is \ncolumn-based, it is an error that the selection is being cleared.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This isn't going to be easy.\n\nEventTableModel is a TableModel, and it must direct the JTable to add or remove columns when the \nTableFormat is changed.\n\nThe JTable class won't add extra columns unless it gets a structure-changed event. And whenever it \nreceives a structure-changed event, the JTable always clears selection.\n\nI have 2 options: \n - report this as a bug in the JDK. The JTable should support the model's column count changing \nwithout the rows changing. This is somewhat appropriate but somewhat bogus.\n - create an elaborate workaround via EventSelectionModel. This will be a hack that causes the \nEventSelectionModel to selectively ignore some clearSelection() method calls.\n\nNeither option is pretty, so I'll let this sit for a while.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've posted a question about this problem on the Swing forums.\nhttp://javadesktop.org/forums/thread.jspa?threadID=7574&tstart=0\n\nThe results of the forum:\n* Swing developer Shan-man suggested that we store the selection before the change and then restore \nit afterwards\n* Jeanette said that it's a limitation of the TableModel's design. It's impossible to change the column \nname in the TableModel because there is no notification path (except the sledge-hammer \nstructureChanged with all those well-known and ugly unwanted side-effects).\n\n\nSo where does this leave me? Well a while ago we added a method to EventSelectionModel called \nsetEnabled() that prevents the ListSelectionModel methods on that class from having any effect \nwhatsoever. All the caller needs to do is find the code that modifies the table's TableFormat, and lock \nthe selection before this method is called. After the method has been returned, the selection can be \nunlocked:\n            issuesSelectionModel.setEnabled(false); // save selection by locking it\n            issuesTableModel.setTableFormat(new IssueTableFormat());\n            issuesSelectionModel.setEnabled(true); // allow selection to be modified\n\nThis is a problem with Swing that we can't fix in an elegant way. As is now, the workaround works \ngreat, so not fixing this should not be a problem.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We should add an issue for this in Sun's bug parade.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've looked through the Sun bug parade. Some bugs to see:\n\n4276786, JTable looses selection when the model fires \"fireTableDataChanged\",\nClosed, not a bug\nhttp://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4276786\n\n4260734, design hole?: TableModelEvent is asymmetric regarding rows and columns\nClosed, will not be fixed\nhttp://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4260734\n\nI doubt that I'm going to have much success, but I'm going to put in an RFE to make TableModelEvent \nsupport columns being added to JTable.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(113));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("DUPLICATE");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("TableFormat.getColumnClass does not exist");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This method is necessary to support different editors. A workaround exists by setting the column editor \nmanually, but this isn't as elegant.\n\nIt might be worthwhile to see whatever other TableFormat stuff can be rolled into a new interface for \nthis, and have the EventTableModel detect that automatically like it does with WritableTableFormat.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Duplicate of 111. How silly of me.\n\n*** This issue has been marked as a duplicate of 111 ***");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(114));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P5);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("UniqueList still uses CompressableList not Barcode.");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Barcode offers huge performance and footprint improvements over \nCompressableList, but currently UnigueList cannot use Barcode to map unique \nentries to the source list.  This is due to UniqueList's use of 3 values in the \ncurrent CompressableList-based implementation.  Since Barcode mandates that \nusers can only use 2 values, this will require UniqueList to be refactored.  As \nit stands, this refactoring appears to be quite painful, so this bug is low-\npriority.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(115));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        currentIssue.setVersion("current");
        currentIssue.setAssignedTo("kevinmaltby");
        currentIssue.setSubcomponent("swt");
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT TableComparatorChooser");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The SWT TableComparatorChooser should be brought up to par with its Swing equivalent.\n\nCommon code between the two classes should be refactored out.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Kevin is now our SWT guy.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Common code has been refactored out and the SWT version is on par with the \nSwing version minus the following beahviour:\n\nThe SWT TableComparatorChooser lacks a parallel to the method \nisSortingMouseEvent().\n\nBut I'm looking into this.\n\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("A parallel method to isSortingMouseEvent() is not possible in SWT.  The only \nTable related widget that a MouseListener can be bound to is the Table itself, \nand interestingly enough it DOES NOT propagate MouseEvents for any clicks that \noccur on the table headers.  Without the use of MouseEvents, you can't obtain \nany speciality information about the event.  For example, which button was used\n(only left mouse clicks cause SelectionEvents or Events), which key modifiers \nwere being held down, whether there were multiple clicks.  All of the above \nmake the isSortingMouseEvent viable and useful to override in a complex rich \nclient app.  SWT doesn't seem to allow this sort of behaviour though.\n\nI am marking this bug as Fixed as a result.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(116));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT Demo application");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The SWT Demo application should be brought up to par with the Swing equivalent.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Well, after a very painful day of delving into SWT further, I almost have the \nSwing demo ported completely to SWT.  For it to be exactly the same, there \nneeds to be a EventListViewer parallel to the EventTableViewer for Lists.  \nCurrently, I've built a clever workaround to present the unique list of users \nand provide filtering based on the selection of those users.  The problem is \nthat workaround isn't very GlazedLists-esque.  But I've got it working.  That's \nstep one.  To get it working correctly, there are at least two more bugs that \nneed to be tossed into the mix.  Time to add those issues...");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Kevin's our new SWT guy!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The shiny new SWT demo is fairly close to done now.  It's getting close to \nbeing a UI match to it's cousin in the Swing package.  It's not yet on par \nfunctionally though.  All of the functionality is present, but some of it is \nhandled in a very non-glazedlists manner.\n\nThe demo currently comes with the following issues:\nA horrible resizing bug on Windows causes description table to collapse.\nThe users table is populated in a brute force way via a custom \nListEventListener rather than an EventListViewer.  (See Issue 133)\nThe description table always auto-scrolls to the end when a different\nissue is selected.\n\nI've made it work.\nTime to make it right.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("To update:\n\nFixed the auto-scrolling issue.\nThe users List is now managed by an EventListViewer.\n\nand I forgot to mention:\nThe demo doesn't die gracefully at all.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Removed some debug output that was an artifact of the old SWT \nTableComparatorChooser.\n\nDemo dies gracefully now.\n\nLeft to do:\nFigure out why SWT collapses my descriptions Table on resizing.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The demo's layout have been totally revamped and the result is a layout that is \nidentical in style of the Swing demo.  At the same time, this eliminated a \nSWT.FILL collision between two tables which had the nasty property of causing \nthe Descriptions table to implode if the window was resized.  The two demos are \nnow identical in form and function with the single exception of a lack of \ngreyed out scroll bars that as far as I can tell is simply an SWT bug.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(117));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT Table should be virtual");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("SWT allows for 'virtual' tables which do not use a TableItem for each row.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Virtual Table support has now been added to EventTableViewer.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(118));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT TextFilterList should be as good as the Swing one");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The SWT TextFilterList should support the same functionality as its Swing equivalent.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I have this almost done.  I still need to add default support for ENTER \ntriggered filtering on a Text component and then it will be on par with the \nSwing version.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The SWT TextFilterList now responds to ENTER in a Text component as a filter \ntrigger for non-live filtering.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(119));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Comparators in a Factory");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Currently to create a ComparableComparator, the user does this: new ComparableComparator(). I think \nit would be better to create a ComparatorFactory that returns various types of Comparators from a \nsimple method call.\n\nclass ComparatorFactory {\n    public Comparator comparable();\n    public Comparator reverse(Comparator forward);\n    public Comparator chain(List comparators);\n}\n\nAnother interesting consequence of this is that we can all of the sudden add new Comparators that the \nusers will like without adding a whole bunch more noise to the Javadocs:\n\n    public Comparator caseInsensitive();\n    public Comparator boolean();\n    public Comparator beanProperty(Class className, String property);");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("This feature also has the nice property of being able to make Singletons of \nComparators that don't have internal state.  While this isn't a big deal, it \ndoesn't really make sense to have more than one ComparableComparator for \nexample, since each instance is exactly the same.  It's not that we couldn't do \nthis before, more that we just didn't, but this gives us a good excuse!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I've got this one.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I've implemented the factory and migrated most of the GlazedLists codebase to \nuse it instead of constructing the comparators directly.  Now here's some \nthoughts on related issues:\n\nI imagine one intent of this enhancement is to move the 3 public comparators \nout of the public scope.  Whether reducing visibility to package scope or \nmoving them into the volatile package, both clear some of the clutter out of \nour JavaDocs and move people into the idea of using our factory.\n\nI was leaning towards putting them into the volatile package so that we still \nhave public access for casting reasons.  The problem is, if users have \nintegrated ReverseComparator and ComparatorChain directly rather than just \nusing them as plain old Comparators via polymorphism, they will suddenly need \nto add a dependency to our volatile package which we want to avoid at all \ncosts.  The classes can't be class-level package scope as that will also break \nusers code with dependencies on those classes and there won't be a workaround.  \nWe can make only the constructors package-level visibility and leave the \nclasses public in the util package.  ComparableComparator we can do whatever we \nwant with as it doesn't extend the Comparator interface at all.\n\nIt seems that the last option is the only real choice at this point.  Thoughts?");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("As per some discussion with Jesse, I opted to move the Comparator \nimplementations into the volatile package as I originally thought.  I'm marking \nthis as fixed although I might beef up the JavaDoc for the ComparatorFactory a \nbit.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(120));
        currentIssue.setStatus("RESOLVED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Beans from a Factory");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Currently there's no way for the user to create a BeanFilterator except via the TextFilterList's \nconstructor. I think we should create a BeanFactory that creates various objects using our BeanProperty \nclass:\n\nclass BeanFactory {\n    public Filterator filterator(Class classname, String[] properties);\n    public Filterator filterator(String[] properties);\n    public TableFormat tableFormat(Class classname, String[] labels, String[] properties, boolean[] \nwritable);\n    public TableFormat tableFormat(String[] labels, boolean[] writable);\n    public Evaluator evaluator(Class classname, String property);\n    public Evaluator evaluator(String property);\n}\n\nThe only really interesting thing about this is that there's also a possibility to use this to create \nComparators. But I just entered another bug about a ComparatorFactory, so who knows! That method \ncould even belong to both.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The code for this is in CVS.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(121));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P1);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("TextFilterList breaks violently if filter goes from 0 to many");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I got an Exception when deleting a backslash character from the TextFilterList. In the demo app, I typed \n\"Invalid\\\" and then deleted the last character:\n\napple.awt.EventQueueExceptionHandler Caught Throwable : java.lang.IndexOutOfBoundsException: \nCannot get at 95 on list of size 95\njava.lang.IndexOutOfBoundsException: Cannot get at 95 on list of size 95\n        at ca.odell.glazedlists.TransformedList.get(TransformedList.java:120)\n        at ca.odell.glazedlists.AbstractFilterList.handleFilterRelaxed(AbstractFilterList.java:200)\n        at ca.odell.glazedlists.DefaultTextFilterList.updateFilter(DefaultTextFilterList.java:131)\n        at ca.odell.glazedlists.DefaultTextFilterList.setFilterText(DefaultTextFilterList.java:94)\n        at ca.odell.glazedlists.swing.TextFilterList.reFilter(TextFilterList.java:226)\n        at ca.odell.glazedlists.swing.TextFilterList.access$200(TextFilterList.java:54)\n        at ca.odell.glazedlists.swing.TextFilterList$FilterEditListener.removeUpdate(TextFilterList.java:206)\n        at javax.swing.text.AbstractDocument.fireRemoveUpdate(AbstractDocument.java:242)\n        at javax.swing.text.AbstractDocument.handleRemove(AbstractDocument.java:607)\n        at javax.swing.text.AbstractDocument.remove(AbstractDocument.java:575)\n        at javax.swing.text.DefaultEditorKit$DeletePrevCharAction.actionPerformed(DefaultEditorKit.java:\n1030)\n        at javax.swing.SwingUtilities.notifyAction(SwingUtilities.java:1535)\n        at javax.swing.JComponent.processKeyBinding(JComponent.java:2438)\n        at javax.swing.JComponent.processKeyBindings(JComponent.java:2473)\n        at javax.swing.JComponent.processKeyEvent(JComponent.java:2401)\n        at java.awt.Component.processEvent(Component.java:4975)\n        at java.awt.Container.processEvent(Container.java:1613)\n        at java.awt.Component.dispatchEventImpl(Component.java:3681)\n        at java.awt.Container.dispatchEventImpl(Container.java:1671)\n        at java.awt.Component.dispatchEvent(Component.java:3543)\n        at java.awt.KeyboardFocusManager.redispatchEvent(KeyboardFocusManager.java:1713)\n        at java.awt.DefaultKeyboardFocusManager.dispatchKeyEvent(DefaultKeyboardFocusManager.java:\n627)\n        at \njava.awt.DefaultKeyboardFocusManager.preDispatchKeyEvent(DefaultKeyboardFocusManager.java:831)\n        at \njava.awt.DefaultKeyboardFocusManager.typeAheadAssertions(DefaultKeyboardFocusManager.java:741)\n        at java.awt.DefaultKeyboardFocusManager.dispatchEvent(DefaultKeyboardFocusManager.java:592)\n        at java.awt.Component.dispatchEventImpl(Component.java:3572)\n        at java.awt.Container.dispatchEventImpl(Container.java:1671)\n        at java.awt.Window.dispatchEventImpl(Window.java:1606)\n        at java.awt.Component.dispatchEvent(Component.java:3543)\n        at java.awt.EventQueue.dispatchEvent(EventQueue.java:456)\n        at java.awt.EventDispatchThread.pumpOneEventForHierarchy(EventDispatchThread.java:234)\n        at java.awt.EventDispatchThread.pumpEventsForHierarchy(EventDispatchThread.java:184)\n        at java.awt.EventDispatchThread.pumpEvents(EventDispatchThread.java:178)\n        at java.awt.EventDispatchThread.pumpEvents(EventDispatchThread.java:170)\n        at java.awt.EventDispatchThread.run(EventDispatchThread.java:100)");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've found that this is the behaviour when deleting any character that causes the list to gain many \nelements.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("It seems like the element count is wrong in these cases.\n\nIn the demo app, type \"tree\" and count the elements. Typing \"treex\" and then \"tree\" should have this \nsame number of elements, but it doesn't. It says \"95\" elements.\n\nWhere this is broken I'm not sure.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Fixed.  Barcode was incorrectly handling the case where a barcode of size n was \nall white and then one value in the middle of the barcode was set to black.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(122));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Optimize Bufferlo");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Bufferlo class is a new data structure contrived to manage lists of ByteBuffers for Glazed Lists on \nthe network.\n\nThis class needs some optimization:\n - better support for mass bytes moved to/from output stream and input/stream\n - ByteBuffer pooling");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(123));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("CTP Bind failure should throw IOException");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Chunk Transfer Protocol CTPConnectionManager class should throw an IOException when bind fails \nduring start(). Currently we bind in several places, and this fails but is not reported. Later we are left \nwondering why we cannot connect!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've posted a question to the Java Forums related to this issue:\nhttp://forum.java.sun.com/thread.jspa?threadID=579248&tstart=0\n\nI think the problem may be fixed for Windows and Linux, but I cannot verify that on my PowerBook. Our \ntest cases fail on my PowerBook.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've posted this to the Mac lists now too:\nhttp://lists.apple.com/archives/java-dev/2004/Dec/msg00350.html");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've opened a peer issue on Apple's bug database. That issue number is \"3922515\"");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Created an attachment (id=8)\nTest case that demonstrates this problem (on Mac OS X)\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This now depends on Apple fixing the JVM bug that is the cause of this. Apple's issue number is \n\"3922515\"\n\nI should receive email notification when the Apple JVM bug is fixed (or not). If it should go unfixed, we'll \nhave to find a workaround. If Apple has no fix within 6 months, we'll build a workaround. Until then, \nwe'll just document the problem and move on.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("A kind soul named Timothy Wall has helped me to work-around the problem. Just call \nsetSoReuseAddress(false) on the channel's socket before binding:\n        ServerSocketChannel channel = ServerSocketChannel.open();\n        ServerSocket socket = channel.socket();\n        socket.setReuseAddress(false);\n        socket.bind(listenAddress);\n\nSee the related thread on the Mac Java mailing list:\nhttp://lists.apple.com/archives/java-dev/2004/Dec/msg00570.html");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've implemented the fix.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(124));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Resource Block Peer needs concurrency strategy");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("ResourceBlockPeer currently does very little locking and concurrency has not been implemented. This \nshould be implemented carefully in a safe way.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I got a bug where the update came before the snapshot:\n        at java.util.ArrayList.add(ArrayList.java:371)\n        at ca.odell.glazedlists.BasicEventList.add(BasicEventList.java:65)\n        at ca.odell.glazedlists.impl.io.ListEventToBytes.toListEvent(ListEventToBytes.java:76)\n        at ca.odell.glazedlists.net.NetworkList$PrivateInterfaces.applyCodedEvent(NetworkList.java:171)\n        at ca.odell.glazedlists.net.NetworkList$PrivateInterfaces.update(NetworkList.java:181)\n        at ca.odell.glazedlists.impl.rbp.PeerResource.remoteUpdate(PeerResource.java:241)\n        at ca.odell.glazedlists.impl.rbp.PeerResource.incomingBlock(PeerResource.java:227)\n        at ca.odell.glazedlists.impl.rbp.PeerConnection.receiveChunk(PeerConnection.java:138)\n        at ca.odell.glazedlists.impl.ctp.CTPConnection.handleChunk(CTPConnection.java:453)\n        at ca.odell.glazedlists.impl.ctp.CTPConnection.handleRead(CTPConnection.java:185)\n        at ca.odell.glazedlists.impl.ctp.CTPSelectorHandler.handleSelectedKeys(CTPSelectorHandler.java:\n65)\n        at ca.odell.glazedlists.impl.ctp.CTPSelectorHandler.run(CTPSelectorHandler.java:39)\n        at ca.odell.glazedlists.impl.ctp.CTPConnectionManager.run(CTPConnectionManager.java:126)\n        at java.lang.Thread.run(Thread.java:552)\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The concurrency strategy is so simple it hurts.\n\nEvery access to the resource side of things must be done on the CTP network thread. I've added some \nmethods to make this easier, namely an invokeLater() method in the RBP Peer class.\n\nThe network always pushes updates to the lists on the network thread. If ever a list needs to write to \nthe network (due to an update event), then this update is marshalled onto the ListPeer thread. This is \nsimple and robust.\n\nThere may be a few places in the code where the order of operations needs to be tuned or state needs \nto be checked before an action, but overall the Network stuff's thread strategy is sound.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(125));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Many calls to Bufferlo.write()");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Many calls in the network code we perform a Bufferlo.write(String). These calls are expensive as each \none needs to convert the String into a byte array, then a byte buffer. Instead the constant Strings should \nbe kept as constant ByteBuffers which can be duplicated when written.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(126));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Bufferlo length is really slow");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Bufferlo length method iterates over the entire bufferlo.\n\nThis should be faster! Perhaps a cache?");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(127));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Automatic reconnect in Resource Block Peer");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Resource Block Peer fails violently if the server disappears before the client has unsubscribed.\n\nInstead the client should handle this gracefully. This should make extensive use of the ResourceStatus \nclass.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've made some significant changes to how this is going to work. Instead of auto-reconnect, the \nNetworkList will simply tell a listener of the disconnect. It is up to the listener to do the reconnect.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Resource Block Peer now goes offline and back online very elegantly. I haven't tested this with failed \nconnections yet, but the basis is ready.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've tested with failed connections and this works great. I have not created a JUnit test for this because \ndoing so is HARD, but I'll think on it further.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("No JUnit test is coming. This works good enough for me.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(128));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Glazed Lists on network packaging");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Glazed Lists on the Network code is scattered haphazardly across several packages including .net, .io \nand .util.impl.\n\nThe CTP code should be in .impl.ctp\nThe RBP code should be in .impl.rbp\nThe ByteCoder code should be in .impl.io\n\nAnd a pretty front end to Peer should be in .io.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The impl package has been divided up!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(129));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("ByteCoder for Serializable");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We need a default ByteCoder, perhaps in a ByteCoderFactory.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Done! Like dinner.\n\nutil.ByteCoderFactory is the new class.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(130));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("ByteCoder for beans");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We should create a ByteCoder that uses java.beans.XMLEncoder. This should be in the \nByteCoderFactory.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This took a whopping four lines of code. DONE!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(131));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Unpublish in Resource Block Peer");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Peer class should support an unpublish() method that withdraws a resource. Clients should \nautomatically reconnect when the service is available again.\n\nClients should receive appropriate notification if their source has disappeared.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This is mostly done. We still need to set up registering and deregistering from the Peer.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Resources are now unpublished. There's even a test case that proves it.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(132));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Resource Block Peer needs P2P");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Resource Block Peer should support load balancing via Peer-to-peer. This may be easy to implement \nbut difficult to implement securely.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(133));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        currentIssue.setVersion("0.9.4");
        currentIssue.setAssignedTo("kevinmaltby");
        currentIssue.setSubcomponent("swt");
        currentIssue.setIssueType("FEATURE");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT Viewer class for Lists");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The SWT package needs a GlazedLists viewer wrapper for Lists parallel to the \none created for Tables.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Created an EventListViewer to bind an EventList to an SWT List Widget.  \nOptionally, a user may specify a custom ListFormat to provide the display value \nshown in the List.  If no ListFormat is specified, elements are displayed as \nthe results of calling toString() on them.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(134));
        currentIssue.setStatus("STARTED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("FEATURE");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT Selection Event List for SWT");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The SWT package needs an event list powered by selection.  It could live inside \npackage scope and only be accessible via the Viewer classes.  The reason I \nthink that this is the best strategy is that SWT doesn't have a Selectable \ninterface despite common selection related methods.  Implementing it directly \nfor the SWT widgets would be more painful.  Working with the Viewer classes, we \nhave a lot more flexibility.  We could have each Viewer class implement a \nSelectable interface of some sort which would proxy calls to the widget that \nthe Viewer wraps simplifying the code in the list itself as it only needs to \nknow the interface.  I think that lends itself to keeping things simple while \noffering up a powerful feature that we enjoy in the Swing package.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("From my modest amount of research, this will be very hard to implement. When table rows are \nremoved, the selection does not move with it. ");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The underpinnings of this feature are in CVS now.  Things left to do:\n\nAdd selection inversion.\nRespond to programmtic selection changes via calls to the selection methods \ndirectly on the wdigets.  ");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(135));
        currentIssue.setStatus("RESOLVED");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("issues@glazedlists");
        currentIssue.setShortDescription("Tutorial in DocBook");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I think that the HTML tutorial has worked well until now. But Docbook is cooler better and more \npowerful. We can use this tool to export our tutorial as PDF as well as HTML.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I'll handle this task over the holidays.  Completing this task will allow me to \npublish my tutorial page for ThresholdList which will bring me within one step \nof being able to FINALLY close Issue 47.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("There's a tool we might want to try out to simplify our documentation-building process:\nhttp://www.cs.put.poznan.pl/dweiss/xml/projects/ant-docbook-styler/index.xml?lang=en\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I've made good progress on this task.  I will complete it when I return from \nthe holidays.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The DocBook formatted tutorial can be found in the www folder for now.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(136));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Keep filter strings as upper case for better performance");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("A large percentage of the text filtering involves converting characters to upper case for case-insensitive \nconversion. It may be faster to convert filter strings to upper case as their element is added to a filter \nlist, then to filter without having to change case.\n\nThis optimization may be better if lazy-loaded.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This is not a DEFECT it's a potential ENHANCEMENT.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(137));
        currentIssue.setStatus("STARTED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("FEATURE");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT Viewer class for Sliders for ThresholdList");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The SWT package needs a GlazedLists viewer wrapper for Sliders.  This is \nnecessary for ThresholdList support out-of-the-box in SWT.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("Checked in the first version of this code.  It has to be tested and validated.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(138));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("ResourceBlockPeer breaks with multiple subscribers");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("When multiple subscribers subscribe to the same source, the resource block peer breaks violently!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("PeerBlock was exhausting the data in an update. Now it reads a duplicate of this data.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(139));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("NetworkList.dispose()");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Disposing a network list should disconnect it and clean up its resources.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("That was easy. Now the dispose() method\n - removes resource status listeners\n - disconnects\n - nulls out the resource status\n - disposes from the source");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(140));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Potential DoS in Network Glazed Lists");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("It is possible to cause an Exception to bubble up in many places in Glazed Lists. The network code, \nespecially the stateful code, should be resistent to errors.\n\nNote that normal operation suffers no known security weaknesses, only malicious connections.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This would be much more compelling to fix with a proper test case.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(141));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("NetworkList could suck up all ram");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("A malicious user could send an open-ended block that would never be processed by NetworkList.\n\nNetworkList should do the following: \n - enforce the maximum amount of time that data can live in the NetworkList without being completed\n - enforce a maximum size of an incoming block\n\nThis is all policy-ish so it should be done very carefully.\n\nNote that this problem only exists for malicious users. Normal operation suffers no such problem.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Yet another test that would benefit greatly from a test case.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(142));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Glazed Lists NetworkLists should have smart URIs");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The URIs for Glazed Lists NetworkLists should be simple and smart.\n\nPublishing should take a paramter such as \"/Contacts\" that automatically works with local resources.\n\nThis will be much harder to do when P2P is in use.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Like dinner, this is done.\n\nNow if you talk to say a computer at 192.168.0.1 and say\n \" Hey monsoon, I'd like to subscribe to monsoon/disco\", then that computer will realize that you think \nyou're talking to a computer called monsoon so it will look for a locally published list called \"/disco\".\n\nNow if you talk to a computer at 192.168.0.1 and say\n\"  Hey tornado, I'd like to subscribe to monsoon/disco\", then that computer will realize that what you're \nasking for is not published locally (tornado != disco), so it would theoretically look in its incoming \nsubscriptions for something called \"monsoon/disco\".\n\nThe key here is that the computer does not know its own hostname. It just infers it from the client's \nrequest. ");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(143));
        currentIssue.setStatus("CLOSED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setAssignedTo("jessewilson");
        currentIssue.setSubcomponent("swing");
        currentIssue.setIssueType("PATCH");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Nasty ClassCastException when user sorts a column with Boolean properties");
        description = new Description();
        description.setWho("mpo");
        description.setWhen(null);
        description.setText("When having a SortedList and connected TableComparatorChooser on a JTable we get a \nnasty ClassCastException when the user tries to sort a column of type Boolean.\n\nThis is caused by the fact that Boolean is not implementing the Comparable\ninterface.\n\nAs a solution we propose not to allow any sorting based on fields which are\nholding the values of not sortable properties.\n\nWe've worked up a stop-gap for this by checking the\neventTableModel.getColumnClass() for being Comparable or not.\n\nIn the long run it might be useful to allow additionally some array of booleans\ndictating which columns are click-sortable and which ones not as a configuration\nsetting to the TableComparatorChooser.\n\nregards,\n-marc=");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I fixed this by adding a method getColumnComparator() to AdvancedTableFormat.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        ...
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(144));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Glazed Lists should recover from RuntimeExceptions");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("When a RuntimeException is thrown by a ListEventListener, the ListEventAssembler should handle the \nproblem intelligently rather than by keeling over and dying.\n\nWhat it could do is finish propagating the event to the existing listeners, finish the commit, and rethrow \nthe exception. Or something.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(145));
        currentIssue.setStatus("RESOLVED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("INVALID");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("WeakReferenceProxy seems out of place.");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("WeakReferenceProxy needs to be public to be accessed by SubList and \nEventListIterator.  However, it doesn't seem to make sense for it to be in the \npublished API for Glazed Lists as it is now.  It is an implementation detail \nthat would likely fit better into the impl package than the event package.\n\nBut I could be wrong and our users could be actively using it in their apps.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I think that it is fair to allow users to use the WeakReferenceProxy. It allows them to forget about having \nto dispose() stuff. It might be better implemented as a WeakReferenceTransformedList, though.\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("See Bug 148.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(146));
        currentIssue.setStatus("STARTED");
        currentIssue.setPriority(Priority.P2);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Cannot deregister Event*Model as Listener");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("You can't deregister the following classes as listeners for an EventList:\n - EventTableModel\n - EventListModel\n - EventSelectionModel\n - EventComboBoxModel\n\nThis prevents these classes from being garbage collected!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've done some work on this, and I decided to (slightly) change our dispose() method strategy.\n\nBefore, dispose() would remove the disposed object as a listener and null our any stateful objects.\nNow it just removes itself as a listener. It doesn't null-out any stateful objects.\n\nThis is because it is possible that some proxies (such as EventThreadProxy) will cause the events to be \nlate, and they will still arrive. This is okay! But if the objects are nulled out, we will either have to check \nfor them or throw a NullPointerException.\n\nA simpler solution is to simply not null the objects out and let the GC do its job.\n\nIn effect, do the simplest thing that will possibly work.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've added dispose() methods for the following Swing classes:\n  EventTableModel\n  EventListModel\n  EventComboBoxModel\n  EventSelectionModel\n\nI've added dispose() methods for the following SWT classes:\n  EventTableViewer\n  EventListViewer\n\nI have not yet completed the following Swing classes:\n  TableComparatorChooser\n  ThresholdRangeModelFactory\n\nI have not yet completed the following SWT classes:\n  SelectionList\n  ThresholdSliderViewerFactory\n  TableComparatorChooser\n \nI'm not sure I need to do all those classes, but it's good to list them so that I can at least know to make \na decision on that.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("swt.ThresholdSliderViewerFactory :\nThe SliderViewers returned by ThresholdSliderViewerFactory act upon a \nThresholdList but they are not registered as ListEventListeners.  The factory \nmethods return SWT SelectionListeners so that the user can deregister the \nSliderViewer from the Slider as needed.\n\nswing.ThresholdRangeModelFactory:\nThe RangeModels returned by the factory methods just act upon a ThresholdList.  \nLike their SWT counterparts, they are not registered as ListEventListeners.\n\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Kevin and I had a chat. We broke out who's responsible for what.\n\nJesse: TableComparatorChoosers (2)\nKevin: SelectionList\n\nThis should be pretty easy!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I'm reasonably certain that the dispose() method on SelectionList was already \nsufficient and correct.  It removes the necessary SWT Listeners and then just \ncalls super.dispose() to clean up the ListEventListener.\n\nThe problem was where to call this and how.  A SelectionList may not actually \nbe used by a user but it still needs to clean up after itself since it is \nalways created.  If the user does use the SelectionLists, it needs to respond \nto a direct call to dispose (or perform a no-op in the case of the deselection \nlist) since they are TransformedLists under the hood and could be casted as \nsuch.  Not a big or likely problem, but it would allow a clever user to shoot \nthemselves in the foot.\n\nI settled on a package-level method called isDisposed() on SelectionList.  \nNothing fancy, but it allows me to guarantee that disposing of the Viewer class \ndisposes the SelectionList as well if the user has not called dispose directly \non the SelectionList.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(147));
        currentIssue.setStatus("RESOLVED");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Swing JTable selection can result in a RuntimeException");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("If you shift-click before making an initial selection on the Issues table in \nthe Swing demo, the following Exception is raised:\n\n\njava.lang.IndexOutOfBoundsException: Invalid range for selection: -1-6, list \nsize is 95\n        at ca.odell.glazedlists.swing.EventSelectionModel.setSubRangeOfRange\n(EventSelectionModel.java:436)\n        at ca.odell.glazedlists.swing.EventSelectionModel.setLeadSelectionIndex\n(EventSelectionModel.java:754)\n        at javax.swing.JTable.changeSelectionModel(Unknown Source)\n        at javax.swing.JTable.changeSelection(Unknown Source)\n...\n\n\nWhile this bug affects the demo, it is primarily a Swing package issue.  \n\nBy default a Swing table shift-clicking on the first click on a JTable just  \nsets the anchor to a valid index.  No selection occurs on that click.  It \nappears that the EventSelectionModel currently tries to select the range with \nan invalid anchor of -1.  \n\nThis problem can only occur on the first click (for the demo) which seems \ninconsistent with default shift-modified selection behaviour for JTables.  The \ndefault shift-modified selection behaviour for any unselected JTable, no matter \nwhat value the anchor may have from previous selections, is to reset the anchor \nvalue (just as was the case on first click).  However, our inconsistent \nimplementation actually seems more natural than the Swing behaviour in that \ncase so I'm not sure if we want to change that while fixing this bug or not.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've changed the EventSelectionModel to clear the selection if the lead is set without the anchor or vice \nversa.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(148));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P5);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("DecoratorList as opposed to TransformedList");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Currently we have a whole bunch of TransformedLists, but I think we need a DecoratorList also.\n\nThe main difference is that DecoratorList doesn't alter the data, it just adds some behaviour. The reason \nthat this is even worth mentioning is that the DecoratorList could forward events in a much more \nefficient way, because they don't need to translate them.\n\nExample: ThreadSafeList.\n\nPotential other examples: WeakReferenceList\n\nThese could even come from a factory.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("ReadOnlyList is another one.\n\nThese should go in a Factory, rather than being fully-exposed classes.\n\npublic EventList readOnly(EventList other)\npublic EventList weakReference(EventList other)\npublic EventList threadSafe(EventList other)");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(149));
        currentIssue.setStatus("RESOLVED");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("issues@glazedlists");
        currentIssue.setShortDescription("Document ListEventPublisher");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This is a black-magic class. I think I should draw something up in OmniGraffle to show how it works!");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Done like dinner. It's not in DocBook or anything, but it is alive.\n\nhttp://publicobject.com/glazedlists/documentation/ListEventPublisher.pdf");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(150));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Demo jar");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("We could create a separate .jar file for the demo apps. Perhaps glazedlists-demo.jar?\n\nThis jar could take a flag ( -swing, -swt) or bring up a dialog to choose a demo.\n\nThe only question is - should we change the path of the demo classes? We could also create subtrees in \nthe existing ca.odell.glazedlists.demo/ package.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("reden");
        description.setWhen(null);
        description.setText("I'll work on this...");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("I like the idea of subtrees within the existing demo package.  It makes the \nmost sense for the SWT IssuesBrowser which shares most of its code with the \nSwing demo.\n\nso maybe a hierarchy something like this?\n\ndemo.issues.swing\ndemo.issues.swt\ndemo.collectionlist\ndemo.network\n...\n\nand so on until we run out of things to show off :)");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've discovered that I need this fixed sooner rather than later for my company's SWT project.\n\nI'm going to move the SWT code immediately, and leave Rob to complete the demo refactor at his \nleisure.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(151));
        currentIssue.setStatus("RESOLVED");
        currentIssue.setPriority(Priority.P2);
        currentIssue.setResolution("FIXED");
        currentIssue.setComponent("glazedlists");
        currentIssue.setVersion("0.9.4");
        currentIssue.setAssignedTo("jessewilson");
        currentIssue.setSubcomponent("demo");
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Swing Demo descriptions table never has any contents.");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The Swing demo's selection works now, but the Description table seems to be \nbroken.  It no longer renders the descriptions for selected items.\n\nAlso, with the demo package getting it's own jar etc, there should be a 'demo' \nsubcomponent to post bugs on.  That way we can specify that a certain Swing, \nSWT or CollectionList bug also affects our public demos which are a significant \nreflection on our project to a new user.  It also has the nice property of \nletting us better target bugs like this one that may have nothing to do with \nglazed lists at all.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("This is now a demo/ issue.");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I'd commented out the line that has the demo using our EventSelectionModel when testing another \nissue, and I had forgotten to restore the original code.\n\nFixed!");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(152));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("FEATURE");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("SWT package lacks Combo support");
        description = new Description();
        description.setWho("kevinmaltby");
        description.setWhen(null);
        description.setText("The SWT package is missing Viewer support for a Combo widget parallel to the \nEventComboBoxModel from the Swing package.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(153));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Observable TableFormat class");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("If AdvancedTableFormat is observable, then we can allow some code to not have to directly talk to the \nEventTableModel.\n\nSee users@ mailing list thread, \"Updating all objects in a list.\"");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Changed from DEFECT to ENHANCEMENT");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(154));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Swing - selection inversion (to be on par with SWT)");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Swing EventSelectionModel should support selection inversion.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(155));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("issues@glazedlists");
        currentIssue.setShortDescription("Tutorial needs updating");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("It's time again to walk through the tutorial. \n\nWe need:\n - a better XSLT for HTML\n - updated screencaps\n - updated concurrency information\n - ListEventPublisher coverage\n - PopularityList coverage\n - CollectionList coverage\n - NetworkList coverage\n\nWe might be able to have an Ant task do all our tutorial work for us. See Issue 135 for details.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(156));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P3);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("NetworkList tests fail");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("Some of the network tests fail on Kevin's Windows 2000 machine:\n\n\nCTPChunkTest fails due to premature closure on testManyStrings().\nIf it doesn't fail for you I will send you the stack trace, but it's\nreally quite long so I won't include it here.\n\nPeerConnectionTest's testPeerConnection fails due to a bind error,\naddress already in use.\n\nCTPConnectionTest fails due to connection refused with no further info.\nHere's the stack trace:    \n\n[junit] WARNING: Closed connection to localhost:0<->localhost:0 due to\njava.net.ConnectException: Connection refused: no further information\n[junit] java.net.ConnectException: Connection refused: no further\ninformation\n[junit]     at sun.nio.ch.SocketChannelImpl.checkConnect(Native Method)\n[junit]     at\nsun.nio.ch.SocketChannelImpl.finishConnect(SocketChannelImpl:521)\n[junit]     at\nca.odell.glazedlists.impl.ctp.CTPConnection.handleConnect(CTPConnection:\n234)\n[junit]     at\nca.odell.glazedlists.impl.ctp.CTPSelectorHandler.handleSelectedKeys(CTPS\nelectorHandler.java:59)\n[junit]     at\nca.odell.glazedlists.impl.ctp.CTPSelectorHandler.run(CTPSelec\ntorHandler.java:39)\n[junit]     at\nca.odell.glazedlists.impl.ctp.CTPConnectionManager.run(CTPCon\nnectionManager.java:127)\n[junit]     at java.lang.Thread.run(Thread.java:534)");
        currentIssue.getDescriptions().add(description);
        description = null;
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've got output from some failing test classes:\n\n    [junit] ------------- Standard Error -----------------\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.StartServer run\n    [junit] INFO: Connection Manager ready, listening on 0.0.0.0/0.0.0.0:5001\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CTPConnection sendResponse\n    [junit] INFO: Accepted connection from localhost:5001<->127.0.0.1:62569\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CTPConnection handleResponse\n    [junit] INFO: Established connection to localhost:62569<->localhost:5001\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] INFO: Closed connection to localhost:62569<->localhost:5001\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] INFO: Closed connection to localhost:5001<->127.0.0.1:62569\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.nio.Shutdown run\n    [junit] INFO: Cleaning up listening socket and closing 2 connections\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.nio.NIODaemon$Stop run\n    [junit] INFO: Server stopping with 0 active connections\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.StartServer run\n    [junit] INFO: Connection Manager ready, listening on 0.0.0.0/0.0.0.0:5002\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CTPConnection sendResponse\n    [junit] INFO: Accepted connection from localhost:5002<->127.0.0.1:62570\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CTPConnection handleResponse\n    [junit] INFO: Established connection to localhost:62570<->localhost:5002\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] INFO: Closed connection to localhost:62570<->localhost:5002\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] INFO: Closed connection to localhost:5002<->127.0.0.1:62570\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.nio.Shutdown run\n    [junit] INFO: Cleaning up listening socket and closing 1 connections\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.nio.NIODaemon$Stop run\n    [junit] INFO: Server stopping with 0 active connections\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.StartServer run\n    [junit] INFO: Connection Manager ready, listening on 0.0.0.0/0.0.0.0:5003\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CTPConnection sendResponse\n    [junit] INFO: Accepted connection from localhost:5003<->127.0.0.1:62571\n    [junit] Jan 19, 2005 12:03:09 AM ca.odell.glazedlists.impl.ctp.CTPConnection handleResponse\n    [junit] INFO: Established connection to localhost:62571<->localhost:5003\n    [junit] Jan 19, 2005 12:03:11 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] INFO: Closed connection to localhost:62571<->localhost:5003\n    [junit] Jan 19, 2005 12:03:11 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] INFO: Closed connection to localhost:5003<->127.0.0.1:62571\n    [junit] Jan 19, 2005 12:03:11 AM ca.odell.glazedlists.impl.nio.Shutdown run\n    [junit] INFO: Cleaning up listening socket and closing 2 connections\n    [junit] Jan 19, 2005 12:03:11 AM ca.odell.glazedlists.impl.nio.NIODaemon$Stop run\n    [junit] INFO: Server stopping with 0 active connections\n    [junit] Jan 19, 2005 12:03:11 AM ca.odell.glazedlists.impl.ctp.StartServer run\n    [junit] INFO: Connection Manager ready, listening on 0.0.0.0/0.0.0.0:5004\n    [junit] Jan 19, 2005 12:03:12 AM ca.odell.glazedlists.impl.ctp.CTPConnection sendResponse\n    [junit] INFO: Accepted connection from localhost:5004<->127.0.0.1:62572\n    [junit] Jan 19, 2005 12:03:12 AM ca.odell.glazedlists.impl.ctp.CTPConnection handleResponse\n    [junit] INFO: Established connection to localhost:62572<->localhost:5004\n    [junit] Jan 19, 2005 12:03:13 AM ca.odell.glazedlists.impl.nio.Shutdown run\n    [junit] INFO: Cleaning up listening socket and closing 2 connections\n    [junit] Jan 19, 2005 12:03:13 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] WARNING: Closed connection to localhost:5004<->127.0.0.1:62572 due to \nca.odell.glazedlists.impl.nio.ServerShutdownException: Server shutting down\n    [junit] ca.odell.glazedlists.impl.nio.ServerShutdownException: Server shutting down\n    [junit]     at ca.odell.glazedlists.impl.nio.Shutdown.run(Shutdown.java:69)\n    [junit]     at ca.odell.glazedlists.impl.nio.BlockingRunnable.run(BlockingRunnable.java:46)\n    [junit]     at ca.odell.glazedlists.impl.nio.NIODaemon.run(NIODaemon.java:81)\n    [junit]     at java.lang.Thread.run(Thread.java:552)\n    [junit] Jan 19, 2005 12:03:13 AM ca.odell.glazedlists.impl.nio.NIODaemon run\n    [junit] SEVERE: Failure processing I/O, continuing\n    [junit] java.lang.IllegalStateException: Closed prematurely! 16 Pending tasks [Expected \"2000:\nMUFLBUDKKALNFUUKNSZHPQYOUSTZOJ\", Enqueued \"3000:WTTSPCPCRRLSCSUGMJJYUWXLUYYNZI\", \nExpected \"2000:TVFRLVXITCJGRPKOVOHLJUCLPZSRMN\", Enqueued \"3000:\nQIOVRPCFPUFDXJLQNCVIFJGRNESSON\", Expected \"2000:JVUJQKSQRMWQXAOESIJJQCGOUQMYOZ\", \nEnqueued \"3000:PLHXESQUXCHUEXGEAEWSVSJWMYRJWV\", Expected \"2000:\nPORNTIUIRBGNAGYSZIJQYLTZBBCMGO\", Enqueued \"3000:PEYTXOQBDVNPHEGLFVVBHJFINZLTHQ\", \nExpected \"2000:ZRGXAWPCSTLIPMBFNIAVCKJYPOVCGV\", Enqueued \"3000:\nZPJGLKPLETSYPEZVPMAFTRDAPPLYQD\", Expected \"2000:UWVQUTBVEXDSOPVKQQAZBOQVVXTEQJ\", \nEnqueued \"3000:ADOHNSTDAXIVVBFQGGRQJUUTJFMYPD\", Expected \"2000:\nJYEOOSCXPNZFWKMZRJEFBGVWUBNCUV\", Enqueued \"3000:PYZNATWIZCTWBCSYNWTNUWJKNAZAHA\", \nExpected \"2000:RQWTQAQAZSYLGSJRTRGGNIOVQEKWPT\", Enqueued \"3000:\nHGKFUVRBMJSNDGYFSZQDNAUQMTHNGI\"], reason \nca.odell.glazedlists.impl.nio.ServerShutdownException: Server shutting down\n    [junit]     at ca.odell.glazedlists.impl.ctp.StaticCTPHandler.connectionClosed(StaticCTPHandler.java:\n98)\n    [junit]     at ca.odell.glazedlists.impl.ctp.CloseConnection.run(CloseConnection.java:87)\n    [junit]     at ca.odell.glazedlists.impl.nio.NIODaemon.run(NIODaemon.java:81)\n    [junit]     at java.lang.Thread.run(Thread.java:552)\n    [junit] Jan 19, 2005 12:03:13 AM ca.odell.glazedlists.impl.ctp.CloseConnection run\n    [junit] WARNING: Closed connection to localhost:62572<->localhost:5004 due to \nca.odell.glazedlists.impl.nio.ServerShutdownException: Server shutting down\n    [junit] ca.odell.glazedlists.impl.nio.ServerShutdownException: Server shutting down\n    [junit]     at ca.odell.glazedlists.impl.nio.Shutdown.run(Shutdown.java:69)\n    [junit]     at ca.odell.glazedlists.impl.nio.BlockingRunnable.run(BlockingRunnable.java:46)\n    [junit]     at ca.odell.glazedlists.impl.nio.NIODaemon.run(NIODaemon.java:81)\n    [junit]     at java.lang.Thread.run(Thread.java:552)\n    [junit] Jan 19, 2005 12:03:13 AM ca.odell.glazedlists.impl.nio.NIODaemon run\n    [junit] SEVERE: Failure processing I/O, continuing\n    [junit] java.lang.IllegalStateException: Closed prematurely! 17 Pending tasks [Expected \"3000:\nWYKBVXSYZQWOJDBANDYMEPQAHHWLNF\", Enqueued \"2000:MUFLBUDKKALNFUUKNSZHPQYOUSTZOJ\", \nExpected \"3000:WTTSPCPCRRLSCSUGMJJYUWXLUYYNZI\", Enqueued \"2000:\nTVFRLVXITCJGRPKOVOHLJUCLPZSRMN\", Expected \"3000:QIOVRPCFPUFDXJLQNCVIFJGRNESSON\", \nEnqueued \"2000:JVUJQKSQRMWQXAOESIJJQCGOUQMYOZ\", Expected \"3000:\nPLHXESQUXCHUEXGEAEWSVSJWMYRJWV\", Enqueued \"2000:PORNTIUIRBGNAGYSZIJQYLTZBBCMGO\", \nExpected \"3000:PEYTXOQBDVNPHEGLFVVBHJFINZLTHQ\", Enqueued \"2000:\nZRGXAWPCSTLIPMBFNIAVCKJYPOVCGV\", Expected \"3000:ZPJGLKPLETSYPEZVPMAFTRDAPPLYQD\", \nEnqueued \"2000:UWVQUTBVEXDSOPVKQQAZBOQVVXTEQJ\", Expected \"3000:\nADOHNSTDAXIVVBFQGGRQJUUTJFMYPD\", Enqueued \"2000:JYEOOSCXPNZFWKMZRJEFBGVWUBNCUV\", \nExpected \"3000:PYZNATWIZCTWBCSYNWTNUWJKNAZAHA\", Enqueued \"2000:\nRQWTQAQAZSYLGSJRTRGGNIOVQEKWPT\", Expected \"3000:HGKFUVRBMJSNDGYFSZQDNAUQMTHNGI\"], \nreason ca.odell.glazedlists.impl.nio.ServerShutdownException: Server shutting down\n    [junit]     at ca.odell.glazedlists.impl.ctp.StaticCTPHandler.connectionClosed(StaticCTPHandler.java:\n98)\n    [junit]     at ca.odell.glazedlists.impl.ctp.CloseConnection.run(CloseConnection.java:87)\n    [junit]     at ca.odell.glazedlists.impl.nio.NIODaemon.run(NIODaemon.java:81)\n    [junit]     at java.lang.Thread.run(Thread.java:552)\n    [junit] Jan 19, 2005 12:03:13 AM ca.odell.glazedlists.impl.nio.NIODaemon$Stop run\n    [junit] WARNING: Server stopping with 2 active connections\n    [junit] ------------- ---------------- ---------------\n    [junit] Testcase: testManyStrings(ca.odell.glazedlists.impl.ctp.CTPChunkTest):      FAILED\n    [junit] 17 uncompleted tasks [Expected \"3000:WYKBVXSYZQWOJDBANDYMEPQAHHWLNF\", Enqueued \n\"2000:MUFLBUDKKALNFUUKNSZHPQYOUSTZOJ\", Expected \"3000:\nWTTSPCPCRRLSCSUGMJJYUWXLUYYNZI\", Enqueued \"2000:TVFRLVXITCJGRPKOVOHLJUCLPZSRMN\", \nExpected \"3000:QIOVRPCFPUFDXJLQNCVIFJGRNESSON\", Enqueued \"2000:\nJVUJQKSQRMWQXAOESIJJQCGOUQMYOZ\", Expected \"3000:PLHXESQUXCHUEXGEAEWSVSJWMYRJWV\", \nEnqueued \"2000:PORNTIUIRBGNAGYSZIJQYLTZBBCMGO\", Expected \"3000:\nPEYTXOQBDVNPHEGLFVVBHJFINZLTHQ\", Enqueued \"2000:ZRGXAWPCSTLIPMBFNIAVCKJYPOVCGV\", \nExpected \"3000:ZPJGLKPLETSYPEZVPMAFTRDAPPLYQD\", Enqueued \"2000:\nUWVQUTBVEXDSOPVKQQAZBOQVVXTEQJ\", Expected \"3000:ADOHNSTDAXIVVBFQGGRQJUUTJFMYPD\", \nEnqueued \"2000:JYEOOSCXPNZFWKMZRJEFBGVWUBNCUV\", Expected \"3000:\nPYZNATWIZCTWBCSYNWTNUWJKNAZAHA\", Enqueued \"2000:RQWTQAQAZSYLGSJRTRGGNIOVQEKWPT\", \nExpected \"3000:HGKFUVRBMJSNDGYFSZQDNAUQMTHNGI\"], pending data \"\"\n    [junit] junit.framework.AssertionFailedError: 17 uncompleted tasks [Expected \"3000:\nWYKBVXSYZQWOJDBANDYMEPQAHHWLNF\", Enqueued \"2000:MUFLBUDKKALNFUUKNSZHPQYOUSTZOJ\", \nExpected \"3000:WTTSPCPCRRLSCSUGMJJYUWXLUYYNZI\", Enqueued \"2000:\nTVFRLVXITCJGRPKOVOHLJUCLPZSRMN\", Expected \"3000:QIOVRPCFPUFDXJLQNCVIFJGRNESSON\", \nEnqueued \"2000:JVUJQKSQRMWQXAOESIJJQCGOUQMYOZ\", Expected \"3000:\nPLHXESQUXCHUEXGEAEWSVSJWMYRJWV\", Enqueued \"2000:PORNTIUIRBGNAGYSZIJQYLTZBBCMGO\", \nExpected \"3000:PEYTXOQBDVNPHEGLFVVBHJFINZLTHQ\", Enqueued \"2000:\nZRGXAWPCSTLIPMBFNIAVCKJYPOVCGV\", Expected \"3000:ZPJGLKPLETSYPEZVPMAFTRDAPPLYQD\", \nEnqueued \"2000:UWVQUTBVEXDSOPVKQQAZBOQVVXTEQJ\", Expected \"3000:\nADOHNSTDAXIVVBFQGGRQJUUTJFMYPD\", Enqueued \"2000:JYEOOSCXPNZFWKMZRJEFBGVWUBNCUV\", \nExpected \"3000:PYZNATWIZCTWBCSYNWTNUWJKNAZAHA\", Enqueued \"2000:\nRQWTQAQAZSYLGSJRTRGGNIOVQEKWPT\", Expected \"3000:HGKFUVRBMJSNDGYFSZQDNAUQMTHNGI\"], \npending data \"\"\n    [junit]     at ca.odell.glazedlists.impl.ctp.CTPChunkTest.testManyStrings(CTPChunkTest.java:163)\n    [junit]     at sun.reflect.NativeMethodAccessorImpl.invoke0(Native Method)\n    [junit]     at sun.reflect.NativeMethodAccessorImpl.invoke(NativeMethodAccessorImpl.java:39)\n    [junit]     at sun.reflect.DelegatingMethodAccessorImpl.invoke(DelegatingMethodAccessorImpl.java:\n25)\n\n\n    [junit] Test ca.odell.glazedlists.impl.ctp.CTPChunkTest FAILED\n");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(157));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P5);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("ENHANCEMENT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("A JavaBeans PropertyListener list");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("I've been doing some reseach on binding for another project, and I came up with a simple idea.\n\nWhat if we had an EventList that automagically updated its elements if they were changed? This could \nuse JavaBeans PropertyChangeListeners.\n\nEaster egg: Good discussion on binding and listeners and observers:\nhttp://martinfowler.com/eaaDev/");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(158));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("TASK");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("New unit-testing minimum requirements");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("After 1.0, we're going to try and validate that everything works as we have already promised. We need \nthe 1.1 bugfix release to contain some bugfixes, and the best way to do this is to find some bugs to fix.\n\nTherefore we're going to institute a \"No EventList held back\" policy where we have minimum \nstandardized testing.\n - JUnit tests for every event list\n - A standard dataset that is really thorough that we run through every EventList, validated against a \nnon-event-driven mockup\n - ConsistencyTestList to verify that EventLists send the change events that they should\n\nThis should be tested upon:\n - SortedList, AbstractFilterList, UniqueList, PopularityList, CollectionList, ThresholdList\n\nThis testing requirement is a big deal and hopefully it will find a small number of shallow bugs rather \nthan a large number of deep bugs.");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
        currentIssue = new Issue();
        currentIssue.setId(new Integer(159));
        currentIssue.setStatus("NEW");
        currentIssue.setPriority(Priority.P4);
        currentIssue.setResolution("");
        currentIssue.setComponent("glazedlists");
        ...
        currentIssue.setIssueType("DEFECT");
        currentIssue.setQAContact("jessewilson");
        currentIssue.setShortDescription("Thread proxies should acquire read locks");
        description = new Description();
        description.setWho("jessewilson");
        description.setWhen(null);
        description.setText("The Thread proxies for SWT and Swing should acquire read locks before they forward their events. Why \nthey don't already is somewhat of a mystery!\n\nThe EventTableModel class doesn't need locking because it has no state, but this isn't the case for any \nof the SWT classes or the EventSelectionModel.\n\nOne thing to note - two changes to an EventList may happen before the proxy gets the read lock for the \nfirst change. This may pose a problem!\n\nAlso note that we can read multiple ListEvents in series if we so choose!");
        currentIssue.getDescriptions().add(description);
        description = null;
        issues.add(currentIssue);
        currentIssue = null;
